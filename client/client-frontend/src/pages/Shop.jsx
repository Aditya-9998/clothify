...
export default function Shop() {
  ...
  // Fetch admin products from Firestore
  useEffect(() => {
  ...
  // Combine local + Firestore products
  const allProducts = [...defaultProducts, ...adminProducts];

  // Filtering
  const filtered = allProducts.filter((product) => {
    const matchesSearch = product.name.toLowerCase().includes(search.toLowerCase());
    const matchesCategory = selectedCategory === "" ? true : product.categories?.includes(selectedCategory);
    ...
    <div className="p-6">
        ...
        {categories.map((cat) => (
          <button
            ...
            className={`px-6 py-2 rounded-full font-medium transition border text-sm
              ${selectedCategory === "" ? (cat === "All" ? "bg-indigo-100 text-indigo-800 border-indigo-200 shadow-md" : "bg-gray-100 text-gray-700 border-gray-200 hover:bg-gray-200") 
              : selectedCategory === cat ? "bg-indigo-100 text-indigo-800 border-indigo-200 shadow-md" : "bg-gray-100 text-gray-700 border-gray-200 hover:bg-gray-200"
            }`}
          >
            {cat}
          </button>
      ...
      {priceRange === "custom" && (
        <div className="flex flex-wrap gap-4 justify-center mb-6">
          <input type="number" placeholder="Min ₹" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} className="border px-4 py-2 rounded w-32" />
          <input type="number" placeholder="Max ₹" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} className="border px-4 py-2 rounded w-32" />
        </div>
      )}
          ...
          return (
            <div key={product.id} className="border rounded-lg shadow p-4 flex flex-col items-center">
              <Link to={`/product/${product.id}`} className="mb-4 block">
                <img src={product.image || tshirtImg} alt={product.name} className="w-48 h-48 object-cover rounded hover:scale-105 transition" />
              </Link>
